...
##### Agent.act
This method returns actions, according to this single policy, for as many input states we wish to pass in (one per agent).  For this project, we will <b>only</b> be calling this method for one agent at a time.  To encourage exploration, the action predicted by the actor_local newtwork has noise added to it.  The amount of noise will decrease as training progresses.  Lastly, the action is clipped to remain within bounds.
##### Agent.step
